...
export function EventDetailView({ event, onBack, onChanged }: EventDetailViewProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md">
      <div className="relative h-56 w-full overflow-hidden bg-[var(--ground-deep)] md:h-72">
        {event.cover_image_url ? (
          <img src={event.cover_image_url} alt={event.title} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[var(--on-ground-faint)]">
      ...
      <div className="mx-auto max-w-5xl px-4 pb-12 md:px-6">
        <div className="sticky top-0 z-10 -mx-4 mb-6 border-b border-[var(--hairline)]/60 bg-black/80 px-4 backdrop-blur-xl md:-mx-6 md:px-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-[2fr_1fr]">
              ...
              <div className="space-y-1 rounded-2xl border border-[var(--hairline)] bg-[var(--ground-deep)]/50 p-6">
                ...
                {!isPublished && (
                  <p className="mt-4 border-t border-[var(--hairline)] pt-4 text-xs text-[var(--on-ground-faint)]">
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--hairline)] bg-[var(--ground-deep)]/50">
            {loading ? (
              ...
              <div className="px-6 py-16 text-center">
                ...
                <p className="mt-1 text-xs text-[var(--on-ground-faint)]">
                  {isPublished
                ...
                {orders.map((o) => (
                  <div key={o.id} className="flex items-center gap-4 px-5 py-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-[var(--on-ground-faint)]">{o.buyer_email}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="text-xs font-bold text-[var(--mint)]">
                        {Number(o.gross_kobo) === 0 ? "Free" : formatKobo(Number(o.gross_kobo))}
                      </p>
                      <p className="text-[10px] text-[var(--on-ground-faint)]">
